...
and returns issues or merge requests created in a given year- either for a
...
1. **GitLab access** (`gitlab_client.py`) - the only file that knows about HTTP, auth headers, and pagination.
...
If either variable is missing, the service fails with a `500` and a clear error message on the first request (see Error Handling below).
...
Issues created in 2026 for a single project (note the path is URL-encoded, `/` becomes `%2F`):
...
> **Note:** querying merge requests / issues across the *entire* gitlab.com instance (no `project` filter) can be slow or time out, since gitlab.com is a massive public instance. Against a real company GitLab instance (fewer projects) this is expected to work well. If GitLab itself times out, the service returns `504 Gateway Timeout`.
...
```
Response: `400 {"detail":"year query parameter is required"}`

**Invalid year format (400):**
```bash
curl -i "http://localhost:8080/issues?year=abcd"
```
Response: `400 {"detail":"'abcd' is not a valid 4-digit year"}`

**Project not found (404):**
```bash
curl -i "http://localhost:8080/issues?year=2026&project=fake-group%2Fno-such-project"
```
Response: `404 {"detail":"GitLab resource not found"}`

**Invalid token (401):**
```bash
curl -i "http://localhost:8080/issues?year=2026"
```
(with an invalid `GITLAB_TOKEN` set) - Response: `401 {"detail":"GitLab authentication failed (invalid token)"}`

**Missing token (500):**
```bash
curl -i "http://localhost:8080/issues?year=2026"
```
(with `GITLAB_TOKEN` not set at all) - Response: `500 {"detail":"GITLAB_URL and GITLAB_TOKEN environment variables are required"}`

## Testing

Tested manually against a real GitLab.com project (`nataly_vaknin-group/nataly_vaknin-project`) with real issues and a real merge request, using a Personal Access Token with `read_api` scope.
